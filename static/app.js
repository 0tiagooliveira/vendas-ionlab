...
const appAssetVersion = "20260617-acessos-contatos-export1";
...
async function renderHomeVendorPages() {
  ...
  if (!isMaster) {
    const userLogin = normalizeSearchText(currentUser?.login || "");
    activeVendors = activeVendors.filter((vendor) =>
      normalizeSearchText(vendor.login_acesso || vendor.login || "") === userLogin
    );
  }
  ...
  `).join("") || `<div class="table-status">${isMaster ? "Nenhum vendedor ativo encontrado." : "Nenhuma página de vendedor vinculada ao seu login."}</div>`;
}
